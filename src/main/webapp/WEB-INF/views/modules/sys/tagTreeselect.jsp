	...
	<script type="text/javascript">
		...
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				...
				data:{simpleData:{enable:true}},callback:{<%--
					...
					top.$("[i-id='ok']").trigger("click");

					<%--top.$.jBox.getBox().find("button[value='确定']").trigger("click");--%>
					<%--<c:if test="${!checked}">--%>
		...
		$(document).ready(function(){
			...
			key = $("#key");
			key.bind("focus", focusKey).bind("blur", blurKey).bind("input change cut  propertychange", searchNode);
...
<body>
	...
	<div id="search" class="form-search " style="padding:10px 0 0 13px;display: none;">
